...
+ (instancetype) createFromTakePictureArguments:(CDVInvokedUrlCommand*)command
{
    ...
    pictureOptions.waterString = [command argumentAtIndex:12 withDefault:nil];
    pictureOptions.dateFormat = [command argumentAtIndex:13 withDefault:nil];
           ...
           [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo
                                    completionHandler:^(BOOL granted) {
                                      if (granted) {
                                        ...
                                        // 拒绝授权
                                          [weakSelf sendNoPermissionResult:command.callbackId withMessage:@"first Not allow"];
                                      }
                                    }];
         } break;
         case AVAuthorizationStatusRestricted:
           // 受限制,家长控制,不允许访问
//            [weakSelf sendNoPermissionResult:command.callbackId withMessage:@"Not allow"];
            [weakSelf sendNoPermissionResult:command.callbackId withMessage:@"show setting"];
...
- (void)sendNoPermissionResult:(NSString*)callbackId withMessage:(NSString *)message
{
    ...
    switch (options.destinationType) {
        case DestinationTypeNativeUri:
        {
            ...
            if (url == nil) {
                ...
                image = [self addTextWatermark:image withWaterString: options.waterString withDateFormat:options.dateFormat];
                //end
